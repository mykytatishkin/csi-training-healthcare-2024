...
namespace CSI.IBTA.Customer.Clients
{
    internal class EnrollmentsClient : IEnrollmentsClient
    {
        ...
        public async Task<GenericResponse<Dictionary<int, decimal>>> GetEnrollmentsBalances(List<int> enrollmentsIds)
        {
            var jsonBody = JsonConvert.SerializeObject(enrollmentsIds);
            var content = new StringContent(jsonBody, Encoding.UTF8, "application/json");
            var response = await _httpClient.PostAsync(BenefitsServiceEndpoints.UpdateEnrollments, content);
            var responseContent = await response.Content.ReadAsStringAsync();
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                var genericError = new HttpError("Something went wrong...", response.StatusCode);
                return new GenericResponse<Dictionary<int, decimal>>(genericError, null);
            }

            var balances = JsonConvert.DeserializeObject<Dictionary<int, decimal>>(responseContent);
